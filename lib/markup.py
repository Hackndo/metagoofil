...
class element:
    ...
    def render(self, tag, single, between, kwargs):
        ...
        if between is not None:
            if isinstance(between, bytes):
                try:
                    between = between.decode('utf-8')
                except:
                    pass
...
class page:
    ...
    def __init__(self, mode='strict_html', case='lower', onetags=None, twotags=None, separator='\n', class_=None):
        ...
        if mode == 'strict_html' or mode == 'html':
            ...
            self.onetags += [t.lower() for t in valid_onetags]
            self.twotags = valid_twotags
            self.twotags += [t.lower() for t in valid_twotags]
            self.deptags = deprecated_onetags + deprecated_twotags
            self.deptags += [t.lower() for t in deprecated_onetags + deprecated_twotags]
            self.mode = 'strict_html'
        elif mode == 'loose_html':
            self.onetags = valid_onetags + deprecated_onetags
            self.onetags += [t.lower() for t in valid_onetags + deprecated_onetags]
            self.twotags = valid_twotags + deprecated_twotags
            self.twotags += [t.lower() for t in valid_twotags + deprecated_twotags]
            self.mode = mode
        elif mode == 'xml':
...
def _argsdicts(args, mydict):
    ...
    mykeys = mydict.keys()
    myvalues = list(map(_totuple, mydict.values()))
